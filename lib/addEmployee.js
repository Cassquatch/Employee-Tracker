...
const addEmployee =  (db) => {
    let role_query = `SELECT title FROM role`;
    let role_array = [];
    ...
    let mgr_query = `SELECT first_name, last_name FROM employee WHERE manager_id is null`;
    let mgr_array = [];
    db.query(mgr_query, (err, data) => {
        if(err) throw err;
        for(let i = 0; i < data.length; i++){
            mgr_array.push(data[i].first_name + " " +  data[i].last_name);
        }
